...
store_views = Blueprint('store_views', __name__, url_prefix='/clouds/api/v1/stores')

STORES_UPLOAD_FOLDER = 'app/static/images/store_images/'

@store_views.route('/', methods=['GET'], strict_slashes=False)
def get_stores():
    """api route to get all stores"""
    stores = current_app.storage.all(Store)
    return jsonify([store.to_dict() for store in stores.values()])

@store_views.route('/', methods=['POST'], strict_slashes=False)
def create_store():
...
@store_views.route('/<store_id>', methods=['PUT'], strict_slashes=False)
...
@store_views.route('/<store_id>', methods=['DELETE'], strict_slashes=False)
...
@store_views.route('/<store_id>', methods=['GET'], strict_slashes=False)
def get_store(store_id):
    """api route to get a store"""
    target_store = validate_model(Store, store_id)
    return jsonify({"success": True, "store": target_store.to_dict()})

@store_views.route('/<store_id>/strains', methods=['PUT'], strict_slashes=False)
...
@store_views.route('/<store_id>/strains', methods=['GET'], strict_slashes=False)
